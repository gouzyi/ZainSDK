...
@implementation AttributeTextModel
@end

@implementation UILabel (YXAction)
...
- (id<YXTextTapActionDelegate>)delegate {
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<YXTextTapActionDelegate>)delegate {
...
- (void)yx_addAttributeTapActionWithStrings:(NSArray<NSString *> *)strings
                                  tapClicked:(void (^)(NSString *, NSRange, NSInteger))tapClick {
    
    [self pm_rangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self pm_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(yx_attributeTapReturnString:range:index:)]) {
        
            [weakSelf.delegate yx_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self gzy_saveEffectDicWithRange:range];
            
            [self gzy_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self pm_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)pm_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock {
    ...
    CGAffineTransform transform = [self gzy_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self gzy_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                AttributeTextModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(gzy_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)gzy_transformForCoreText {
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)gzy_getLineBounds:(CTLineRef)line point:(CGPoint)point {
    CGFloat ascent = 0.0f;
...
- (void)gzy_tapEffectWithStatus:(BOOL)status {
    if (self.isTapEffect) {
...
- (void)gzy_saveEffectDicWithRange:(NSRange)range{
...
- (void)pm_rangesWithStrings:(NSArray <NSString *>  *)strings {
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf pm_getStringWithRange:range]];
            
            AttributeTextModel *model = [AttributeTextModel new];
...
- (NSString *)pm_getStringWithRange:(NSRange)range {
